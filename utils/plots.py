...
class PlotManager(object):
    ...
    def plot_scatter_with_shapes(self, cell, metric_name, metric):
        ax = plt.subplot(cell)
        ax.set_xticks([])
        ax.set_yticks([])
        ...
        for metric_subgroup, marker in zip(metric['data'], ['o', 'v', ',']):
            category_labels = metric_subgroup[..., 2]
            unique_labels = list(set(category_labels))
            norm = colors.Normalize(vmin=0, vmax=len(unique_labels))
            for i, label in enumerate(unique_labels):
                category_labels = [i if x == label else x for x in category_labels]
            ...
            ax.scatter(metric_subgroup[:, 0], metric_subgroup[:, 1],
                       color=mapped_colors,
                       label=unique_labels, alpha=0.7, marker=marker,
                       edgecolors='none',
                       s=16)
        patch = mpatches.Patch(color='silver', label=metric_name)
        ax.legend(handles=[patch], prop={'size': 16})
    ...
    def plot(self, outfile, wspace=None, hspace=None, figsize=8, dpi=200):
        ...
        for j, metric_name in enumerate(sorted(self.to_plot)):
            ...
            elif metric['data_type'] == 'scatter-with-shapes':
                self.plot_scatter_with_shapes(current_cell, metric_name, metric)
