...
class ExtractFeaturesFlipped(Experiment):
    name = "extract-features-flipped"
    requires_model = True
    ...
    def gather_sketches_for(self, set_type):
        ...
        filename = os.path.join(self.hps['results_dir'], "{}_{}_{}_feats_flipped.npz".format(model_name, dataset_name, set_type))
        ...
        for batch in batch_iterator:
            ...
            crops = utils.bbox.crop_bbox_batch(
                imgs, boxes, objs_to_img, self.model.dataset.hps['crop_size'])

            # flip them all
            flip_crops = tf.image.flip_left_right(crops)
            flip_sketches = tf.image.flip_left_right(sketches)

            skt_comp = utils.layout.get_sketch_composition(
                flip_sketches.numpy(), boxes.numpy(), objs_to_img, img_s, img_s, batchs, avoid_overlap=True)

            img_reps, obj_co_vecs = self.model.inference_image_representation_from_crop(
                batchs, flip_crops, triples, boxes, attr, objs_to_img)
            img_skt_reps, skt_co_vecs = self.model.inference_sketch_representation(
                batchs, triples, boxes, attr, objs_to_img, flip_sketches)
        ...
        data = {'sketches': all_skts[:n_samples],
                'img_reps': all_reps[:n_samples],
                'skt_reps': all_skt_reps[:n_samples],
                'images': all_imgs[:n_samples],
                'objs': all_objs[:n_samples],
                'sketch_obj_vecs': all_svecs[:n_samples],
                'image_obj_vecs': all_ovecs[:n_samples],
