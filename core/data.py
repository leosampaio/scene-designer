...
class BaseDataLoaderV2(object, metaclass=ABCMeta):

    def __init__(self, hps):
        if hps is None:
            hps = self.default_hparams()
...
class BaseDataLoader(object, metaclass=ABCMeta):
    ...
    def __init__(self, hps, data_directory, evaluate_only=False, train_only=False):
        ...
        for s, split in self.splits.items():
            ...
            if evaluate_only:
                if s != 'train':
                    ...
                    split.thread.start()
                    self._swap_used_set_for_preloaded_one(s)
                    self.splits[s].did_load_first_megabatch = True
            elif train_only:
                if s == 'train':
                    split.thread = threading.Thread(
                        target=self.load_next_megabatch, args=(s, selected_file))
    ...
    def _swap_used_set_for_preloaded_one(self, split_name):
        ...
        #         break
        #     except OSError:
        #         print("[DATA] Tried to load {} and failed".format(selected_file))
    ...
    def batch_iterator(self, split_name, batch_size, stop_at_end_of_split=False, preprocess=True, **kwargs):
        ...
        """
        print("[DATA] Created batch iterator")
        if not isinstance(split_name, str):
            split_name = split_name.decode()
        iterator_status = 'running'
        while not stop_at_end_of_split or not iterator_status == 'end_of_split':
            samples = []
            self.splits[split_name].lock.acquire()
            ...
            self.splits[split_name].lock.release()
            if preprocess:
                yield self.preprocess_batch(samples, split_name, **kwargs)
            else:
                yield samples

    def tf_batch_iterator(self, split_name, batch_size, stop_at_end_of_split=False, **kwargs):
        return tf.data.Dataset.from_generator(
            self.batch_iterator, args=(split_name, batch_size, stop_at_end_of_split),
    ...
    def get_n_samples_from(self, split_name, n, shuffled=False, seeded=None, preprocess=False):
        ...
        if preprocess:
            return self.preprocess_batch(samples, split_name)
        else:
            return samples
            ...
            for idx in indices[i:end_idx]:
                ...
                for i, element in enumerate(sample):
                    if len(samples) == i:
                        ...
                        samples[i].append(element)
            samples = [np.array(element) for element in samples]
            yield self.preprocess_batch(samples, split_name)

    def tf_get_n_samples_batch_iterator_from(self, split_name, n, batch_size, shuffled=False, seeded=None, preprocess=False, **kwargs):
        return tf.data.Dataset.from_generator(
            self.get_n_samples_batch_iterator_from, args=(split_name, n, batch_size, shuffled, seeded, preprocess),
            output_types=self.output_types, output_shapes=self.output_shapes).repeat()
    ...
    def get_all_data_from(self, split_name):
        ...
        return self.preprocess_batch(self.get_n_samples_from(
            split_name, n=self.splits[split_name].current_len), split_name)

    def preprocess_batch(self, samples, split_name):
