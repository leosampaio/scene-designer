...
class SampleKL(tf.keras.layers.Layer):
    ...
    def __init__(self, stddev=1, **kwargs):
        super(SampleKL, self).__init__(**kwargs)
        self.stddev = stddev
        self.result = None
    ...
    def call(self, inputs, training=True):
        ...
        if training:
            epsilon = tf.random.normal(shape=tf.keras.backend.shape(z_mean), mean=0, stddev=self.stddev)
            self.result = z_mean + tf.exp(0.5 * z_log_var) * epsilon
        else:
            self.result = z_mean
        return self.result
...
class KLDivergence(tf.keras.layers.Layer):
    ...
    def __init__(self, emb_dim, dec_inter_dim, dec_dim, stddev=1, **kwargs):
        super(KLDivergence, self).__init__(**kwargs)
        self.emb_dim = emb_dim
        self.dec_inter_dim = dec_inter_dim
        self.dec_dim = dec_dim
        self.stddev = stddev

        self.mean_layer = tf.keras.layers.Dense(self.emb_dim)
        self.logvar_layer = tf.keras.layers.Dense(self.emb_dim)
        self.dec_inter_layer = tf.keras.layers.Dense(self.dec_inter_dim, activation='relu')
        self.dec_layer = tf.keras.layers.Dense(self.dec_dim)
    ...
    def call(self, x, training=True):
        ...
        if training:
            ...
            z = z_mean
        dec_output = self.dec_inter_layer(z)
        dec_output = self.dec_layer(dec_output)
        return z_mean, z_logvar, z, dec_output

    def compute_output_shape(self, input_shape):
        base_shape = list(input_shape[:-1])
        return base_shape + [self.emb_dim], base_shape + [self.emb_dim],\
            base_shape + [self.emb_dim], base_shape + [self.dec_dim]
...
class Encoder(tf.keras.layers.Layer):
    ...
    def __init__(self, num_layers, d_model, num_heads, dff,
                 maximum_position_encoding, input_vocab_size=None, rate=0.1,
                 use_continuous_input=False, custom_pos_encoding=None):
        ...
        elif input_vocab_size is not None:
        ...
        if custom_pos_encoding is None:
            self.pos_encoding = positional_encoding(maximum_position_encoding,
                                                    self.d_model)

            def _add_pos_encoding(x, seq_len):
                return x + self.pos_encoding[:, :seq_len, ...]
            self.add_pos_encoding = _add_pos_encoding
        else:
            self.add_pos_encoding = custom_pos_encoding
    ...
    def call(self, x, training, mask):
        ...
        x = self.add_pos_encoding(x, seq_len)
