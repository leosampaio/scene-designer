...
class COCOTFRecord(BaseDataLoaderV2):
    name = 'coco-tfrecord'

    @classmethod
    def default_hparams(cls):
        hps = utils.hparams.HParams(
            coco_dir='/store/lribeiro/datasets/coco-tfrecord',
    ...
    def __init__(self, hps=None):
        ...
        for split in ['train', 'valid', 'test']:
            tfrecords_pattern = os.path.join(self.hps['coco_dir'], "{}*.npz".format(split))
    ...
    def get_iterator(self, split_name, batch_size, shuffle=None, prefetch=5, repeat=False):
        ...
        mix_batch, out_types = self.build_mix_batch(qditerators, cciterators)
    ...
    def build_mix_batch(self, qditerators, cciterators):
        def mix_batch(n_objs, n_triples, images, objs,
                      boxes, masks, triples, attributes, ids):
            ...
            for i, obj_y in enumerate(f_objs):
                sketches[i], _ = next(qditerators[self.obj_idx_to_name[obj_y[0]]])
            ...
            return images, f_objs, f_boxes, f_masks, f_triples, f_attr, objs_to_img, sketches, neg_crops, neg_labels, ids
