...
class SketchyCOCOTFRecord(BaseDataLoaderV2):
    name = 'sketchycoco-tf'

    @classmethod
    def default_hparams(cls):
        hps = utils.hparams.HParams(
            sketchycoco_dir='/scratch/lribeiro/datasets/sketchycoco-tf',
    ...
    def __init__(self, hps=None):
        ...
        with open(os.path.join(self.hps['sketchycoco_dir'], "meta.json")) as f:
        ...
        for split in ['train', 'valid']:
            tfrecords_pattern = os.path.join(self.hps['sketchycoco_dir'], "{}-*.records".format(split))
    ...
    def get_iterator(self, split_name, batch_size, shuffle=None, prefetch=5, repeat=False):
        ...
        dataset = dataset.map(
            utils.tfrecord.parse_sketchycoco_scene_graph_record, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    ...
    def build_mix_batch(self, cciterators):
        def mix_batch(n_objs, n_triples, images, objs,
            ...
            return images, f_objs, f_boxes, f_masks, f_triples, f_attr, objs_to_img, f_sketches, neg_crops, neg_labels, ids
...
def main():
    ...
    qdset = SketchyCOCOTFRecord()

    start_time = time.time()
    iterator = qdset.get_iterator('valid', 16, shuffle=50, repeat=True)
    counter = 0
